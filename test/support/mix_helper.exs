...
defmodule MixHelper do
  ...
  def in_tmp_project(which, function) do
    conf_before = Application.get_env(:commanded, :generators) || []
    path = Path.join([tmp_path(), random_string(10), to_string(which)])
    ...
    after
      File.rm_rf!(path)
      Application.put_env(:commanded, :generators, conf_before)
    end
  end
  ...
  def with_generator_env(app_name \\ :commanded, new_env, fun) do
