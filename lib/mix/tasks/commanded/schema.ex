defmodule Mix.Commanded.Schema do
  @moduledoc false
  ...
  def new(schema_name, schema_plural, cli_attrs, opts) do
    ...
    basename  = CmdGen.Naming.underscore(schema_name)
    ...
    %Schema{
      ...
      human_plural: CmdGen.Naming.humanize(schema_plural),
  ...
  * In case it is a boolean, we keep it as false
  ...
  For example:
      ...
      mix cmd.gen.schema Comment comments body:text status:enum:published:unpublished
  """

  defp validate_attr!({name, :datetime}), do: validate_attr!({name, :naive_datetime})
  defp validate_attr!({name, :array}) do
    Mix.raise """
    Commanded generators expect the type of the array to be given to #{name}:array.
    For example:

        mix cmd.gen.schema Post posts settings:array:string
    """
  end
  ...
  defp partition_attrs_and_assocs(schema_module, attrs) do
    {assocs, attrs} =
      Enum.split_with(attrs, fn
        ...
        {key, :references} ->
          Mix.raise """
          Commanded generators expect the table to be given to #{key}:references.
          For example:

              mix cmd.gen.schema Comment comments body:text post_id:references:posts
          """
        _ -> false
      end)
    ...
    assocs =
      Enum.map(assocs, fn {key_id, {:references, source}} ->
        ...
        module = Module.concat(base ++ [CmdGen.Naming.camelize(key)])
